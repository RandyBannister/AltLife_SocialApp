...
- (void)setupCellWithType:(AMBubbleCellType)type withWidth:(float)width andParams:(NSDictionary*)params
{
    ...
    textFont = [UIFont fontWithName:@"ArialMT" size:16.0];
	...
	if (type == AMBubbleCellSent) {
        [self.textView setTextColor:[UIColor colorWithRed:220.0f/255.0f green:220.0f/255.0f blue:220.0f/255.0f alpha:1.0f]];

		[self.bubbleAccessory setFrame:CGRectMake(width - self.bubbleAccessory.frame.size.width - 20,
												  2,
												  self.bubbleAccessory.frame.size.width+20,
												  self.bubbleAccessory.frame.size.height)];
		
		
		CGRect rect = CGRectMake(width - sizeText.width - 34.0f - self.bubbleAccessory.frame.size.width-20,
								 textFont.lineHeight - 13.0f,
								 sizeText.width + 54.0f,
								 sizeText.height + 32.0f);
	...
	if (type == AMBubbleCellReceived) {
		...
		[self.bubbleAccessory setFrame:CGRectMake(20,
												  2,
												  self.bubbleAccessory.frame.size.width+20,
												  self.bubbleAccessory.frame.size.height)];
		...
		CGRect rect = CGRectMake(0.0f + self.bubbleAccessory.frame.size.width,
								 textFont.lineHeight - 13.0f,
								 MAX(sizeText.width, usernameSize.width) + 54.0f, // Accounts for usernames longer than text
								 sizeText.height + 32.0f + usernameSize.height);
		...
		[self setupBubbleWithType:type
					   background:rect
						textFrame:CGRectMake(32.0f, 14.0 + usernameSize.height, sizeText.width + 5.0f, sizeText.height)
						  andText:params[@"text"]];
	}
...
- (void)setupBubbleWithType:(AMBubbleCellType)type background:(CGRect)frame textFrame:(CGRect)textFrame andText:(NSString*)text
{
	...
	if (type == AMBubbleCellReceived) {
        ...
        UIImage *ima =[UIImage imageNamed:@"receiveBubble.png"];
		[self.imageBackground setImage:ima];
	} else {
        UIImage *ima =[UIImage imageNamed:@"sendBubble.png"];
        [self.imageBackground setImage:ima];	}
